...
int new_file(terminal_context_t *context, char *path, permission_t perms)
{
    ...
    if(last_len < MAX_FILE_NAME_LEN){
        memcpy(entry + 2, last, last_len);
    } 
    else{
...
int list(terminal_context_t *context, char *path)
{
    ...
    inode_t *parent = NULL;
    if(token_count > 1){
        fs_retcode_t retcode = followPath(context->fs, context->working_directory,
                                          tokens, token_count - 1, &parent);
        if(retcode == INVALID_FILE_TYPE) {
            REPORT_RETCODE(INVALID_FILE_TYPE);
            free(buffer);
            return -1;
    ...
    if(dis_node->internal.file_type == DATA_FILE){
        ...
        sprintf(line, "%c%c%c%c\t%lu\t%s", indicate_type, indicate_readable,
                indicate_writable, indicate_exec,
                (unsigned long)dis_node->internal.file_size,
                dis_node->internal.file_name);
        puts(line);
    } 
    else{
        size_t e_read = 0;
        ...
        for(size_t i = 0; i < e_read; i++){
            ...
            printf("%c%c%c%c\t%lu\t%s", indicate_type, indicate_readable,
                   indicate_writable, indicate_exec,
                   (unsigned long)child->internal.file_size,
                   namebuf);
...
char *get_path_string(terminal_context_t *context)
{
    ...
    for(size_t i = 0; i < depth; i++){
        total_len += strlen(names[i]) + 1; // for /
    }
